    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-800 to-gray-900">
      <div className="container mx-auto px-3 sm:px-4 py-4 sm:py-8 max-w-7xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8 items-start">
          ...
          <div className="order-2 lg:order-1">
            ...
            {fireState.lastAction && (
              <div className="mt-4 sm:mt-6 flex justify-center">
                <button
                  onClick={handleUndo}
                  className="px-4 sm:px-6 py-2 sm:py-3 bg-red-600/20 hover:bg-red-600/30 border border-red-500/50 text-red-300 rounded-lg text-sm sm:text-base font-semibold transition-all duration-200 hover:scale-105 flex items-center gap-2"
                >
                  <span>↶</span>
                  <span className="truncate max-w-[200px] sm:max-w-none">Undo: {fireState.lastAction.label}</span>
                </button>
              </div>
            )}
          </div>
          ...
          <div className="order-1 lg:order-2 flex flex-col items-center justify-center bg-gray-800/20 backdrop-blur-sm border border-gray-700/30 rounded-2xl p-4 sm:p-8 min-h-[300px] sm:min-h-0">
            <FireVisualization fireLevel={fireState.fireLevel} />
          </div>
        </div>

        {/* Info Box and History - Side by Side */}
        <div className="mt-6 sm:mt-12 grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 max-w-7xl mx-auto">
          {/* Info Box */}
          <div className="bg-gray-800/20 backdrop-blur-sm border border-gray-700/30 rounded-xl p-4 sm:p-6">
            <h3 className="text-white font-semibold text-base sm:text-lg mb-3">How It Works</h3>
